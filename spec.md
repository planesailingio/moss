...
**Windows** has no per-open containment. Traverse component by component, checking `FILE_ATTRIBUTE_REPARSE_POINT` and the reparse tag (`IO_REPARSE_TAG_SYMLINK` vs `IO_REPARSE_TAG_MOUNT_POINT`). Verify containment by comparing volume serial and file index from `GetFileInformationByHandle` — **never by string prefix**. `FILE_FLAG_OPEN_REPARSE_POINT` opens the reparse point itself and needs `FILE_FLAG_BACKUP_SEMANTICS` for directories. The documented limit is 63 reparse points per path.
