...
|---|---|
| `cli/` | clap definitions (`mod.rs`: global flags and the command enum) and one file per command group: `init`, `doctor`, `inspect`, `backup`, `snapshots`, `restore`, `maintenance` (`status`, `verify`, `prune`, `maintenance`), `misc` (`config`, `include`, `exclude`, `recovery`, `yubikey`, `kopia`). `doctor` wires `crate::doctor` to the invocation and renders the report. `context.rs` holds per-invocation state: `Options` (the behavioural global flags), the injectable `store_factory` and `kopia_factory` seams, and the connect path. |
...
| `backup/` | `kopia.rs`: the `KopiaRunner` trait and `KopiaContext`, its real implementation (subprocess spawn, version pin, environment allowlist, stderr forwarded line by line to the `kopia` tracing target, a 60 s timeout on metadata verbs, stderr classification); `FixtureRunner` under `cfg(test)` answers from the captured fixtures. `json.rs`: defensive views of Kopia's `--json`. `repository.rs`: typed operations over a `&dyn KopiaRunner` (create, connect, status, policy, snapshot create/list/restore/verify/expire, maintenance). `run.rs`: the backup run. `manifest.rs`: the per-run manifest. `tags.rs`: the tag scheme. |
...
| `doctor/` | The `moss doctor` checks as a library: `Check`, `CheckStatus` and `Report` (serialisable, with `exit_code()`), and one function per check (Kopia present and in range, credentials and store probe, repository reachable, Kopia config file mode, recovery acknowledged, Full Disk Access, session, state directory, YubiKey, scan-index age). Each takes explicit inputs so it is testable without a terminal or a Kopia. |
